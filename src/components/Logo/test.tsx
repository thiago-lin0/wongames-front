...
describe('<Logo />', () => {
  it('should render a white label by default', () => {
    ...
    expect(screen.getByLabelText(/Won Games/i).parentElement).toHaveStyleRule('color: #FAFAFA')
  })

  it('should render a black label when color is passsed', () => {
    renderWithTheme(<Logo color='black'/>)
    expect(screen.getByLabelText(/Won Games/i).parentElement).toHaveStyleRule('color: #030517')
  })

  it('should render a bigger logo', () => {
    renderWithTheme(<Logo size="large" />)
    expect(screen.getByLabelText(/Won Games/i).parentElement).toHaveStyleRule('width: 20rem')
  })

  it('should render a normal logo when size is defaul', () => {
    renderWithTheme(<Logo/>)
    expect(screen.getByLabelText(/Won Games/i).parentElement).toHaveStyleRule('width: 11rem')
  })
